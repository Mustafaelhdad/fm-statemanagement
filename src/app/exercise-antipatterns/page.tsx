...
function BookingTimer() {
  ...
  const timerIdRef = useRef<NodeJS.Timeout | null>(null); // ❌ Should use useRef
...
function HotelGallery() {
  ...
  const lastScrollPositionRef = useRef<number>(0); // ❌ Should use useRef
...
function FlightSearch() {
  ...
  >([]);
  const searchCountRef = useRef<number>(0); // ❌ Should use useRef
  const lastSearchTimeRef = useRef<number | null>(null); // ❌ Should use useRef

  const handleSearch = async () => {
    const now = Date.now();

    // Track search analytics (doesn't affect UI)
    searchCountRef.current = searchCountRef.current + 1; // ❌ Unnecessary re-render
        ...
        <div className="space-y-2">
          {hotels.map((hotel) => (
            <div
              ...
              onClick={() => handleHotelSelect(hotel.id as string)}
            >
              <div className="flex items-center justify-between">
                <div>
          ...
          <div className="border-t pt-4">
            ...
            <div className="mt-3">
              <Badge variant="destructive" className="text-xs">
                Storing entire object with{" "}
                {Object.keys(selectedHotel ?? {}).length} properties
              </Badge>
            </div>
          </div>
        )}
...
function BookingSummary() {
  ...
  const formattedDepartureDate = depDate.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
  });
  const formattedReturnDate = retDate.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
  });
